...
7.  Giải mã: $\math{M} = C^{d} \mod N$.

Chứng minh $\math{M} = M$: <br/>
Ta có $d = e^{-1} \pmod{\phi(N)} \Rightarrow ed = \phi(N).t + 1$. <br/>
$\math{M} = C^{d} \mod N = M^{ed} \mod N = M^{\phi(N).t + 1} \mod N = M^{(p-1).(q-1).t + 1} \mod N$. Theo định lý Fermat ta có: $M^{(p-1).(q-1).t + 1} \equiv M \pmod{N} \Rightarrow \math{M} \mod N = M \mod N = M$ (Vì $M < N$)
## 
